defmodule ToxicParser.ContainersTest do
  ...
  test "parses empty list and trailing comma" do
    ...
    log = EventLog.new()
    assert {:ok, [], _state, _cursor, _log} = Grammar.Expressions.expr(state, cursor, Context.matched_expr(), log)

    {state, cursor} = TokenAdapter.new("[1,]")
    assert {:ok, [1], _state, _cursor, _log} = Grammar.Expressions.expr(state, cursor, Context.matched_expr(), log)
  end
