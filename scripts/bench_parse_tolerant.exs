#!/usr/bin/env elixir
# Phase 0 benchmark: Tolerant mode parser throughput
#
# Measures ToxicParser performance in tolerant mode with token_metadata.
# Tolerant mode never crashes and always returns best-effort AST + diagnostics.
# Tracks: wall time, reductions, GC counts, memory.
#
# Usage:
#   mix run scripts/bench_parse_tolerant.exs
#   mix run scripts/bench_parse_tolerant.exs --project elixir
#   mix run scripts/bench_parse_tolerant.exs --limit 100
#   mix run scripts/bench_parse_tolerant.exs --runs 10

defmodule BenchParseTolerant do
  ...
  def run(args) do
    ...
    IO.puts("=== Bench: Parser Tolerant Mode ===")
    ...
    results =
      for run <- 1..opts.runs do
        ...
        # Run workload
        {time_us, {node_count, diag_count}} = :timer.tc(fn -> parse_all(contents) end)
        ...
        diag_info = if diag_count > 0, do: " (#{diag_count} diagnostics)", else: ""
        ...
        IO.puts(
          "Run #{run}: #{Float.round(time_ms, 2)} ms | " <>
            ...
            "#{gc_count} GCs | " <>
            "#{div(gc_words * 8, 1024)} KB reclaimed#{diag_info}"
        )
    ...
    diag_count = hd(results).diagnostics
    ...
    IO.puts("Diagnostics: #{diag_count}")
  ...
  defp parse_all(contents) do
    Enum.reduce(contents, {0, 0}, fn content, {nodes, diags} ->
      # Tolerant mode always succeeds with {:ok, result}
      case ToxicParser.parse_string(content, mode: :tolerant, token_metadata: true) do
        {:ok, result} ->
          count = count_ast_nodes(result.ast)
          diag_count = length(Map.get(result, :diagnostics, []))
          {nodes + count, diags + diag_count}

        {:error, result} ->
          # Even on error, we get an AST in tolerant mode
          count = count_ast_nodes(Map.get(result, :ast))
          diag_count = length(Map.get(result, :diagnostics, []))
          {nodes + count, diags + diag_count}
      end
    end)
...
BenchParseTolerant.run(System.argv())
